$(document).ready(function(){
  ...
  skratch.bgcanvas = document.getElementById('bg_canvas');

  skratch.canvas.width = skratch.width;
  skratch.canvas.height = skratch.height;
  skratch.bgcanvas.width = skratch.width;
  skratch.bgcanvas.height = skratch.height;

  skratch.context = skratch.canvas.getContext('2d');
  skratch.bgcontext = skratch.bgcanvas.getContext('2d');
  ...
  // Setup the color picker elements
  ...
  var panel_exclusive = false
  ,   single_panel = false
  ,   panel_width = 320
  ;
  
  var size_panels = function() {
      ...
      if (single_panel && $('#left-pane').data('open') && $('#right-pane').data('open')) {
        $('#right-pane').data('open', false)
      }
  ...
  $(window).resize(function() { size_panels() });
  ...
  $('#tool-options').mousedown(function(){ skratch.setColor(true) });

  $('#tool-options').mouseup(function(){ skratch.setColor(false) });

  $('#tool-options').mouseleave(function(){ skratch.setColor(false) });
  ...
  var touch_event = function(e) {
    ...
    if (e.originalEvent.touches[1] || e.originalEvent.touches[1] === 0){
      ...
      if (pan.scale) { skratch.zoom(e.originalEvent.scale * pan.scale) }
  ...
  (function(){
    thistip = 0; skratch.tooltipsON = true;
...
var log = function(message) { $('#debug').html('<p>' + message + '</p>' + $('#debug').html()) };
