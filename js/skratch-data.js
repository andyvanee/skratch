  ...
  skratch.data = {
    ...
    restore : function(index) {
      ...
      if (thisimage.length > 0) {
        ...
        restored.onload = function() { skratch.drawImage(restored) };
      } else {
    ...
    renderIconPane : function () {
      ...
      for (var i=0; i < skratch.data.files.length; i++) {
        var iconImageTag = "<img data-index='" +
                           i +
                           "' src='" +
                           localStorage.getItem(skratch.data.files[i].icon) +
                           "'>"
                           ;
        $('#icon_pane').append(iconImageTag);
      }
    ...
    getBackground : function(index) {
      $.getJSON("backgrounds/" + index + "tn.json", function(data, textStatus){
        ...
        var $target_img = $("#backgrounds-pane img[data-index='" + data['index'] + "']");
        
        $target_img.attr("src", data["image"]);
        ...
        if (skratch.backgroundsLoaded < 9) { skratch.data.getBackground(skratch.backgroundsLoaded) }
    ...
    files : (function() {
      ...
      if( filelist !== null) { return jQuery.parseJSON(filelist) }
      else { return [] }
